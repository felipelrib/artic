...
import { Space, Divider, Title, Container, Text } from '@mantine/core';
...
export default function Album({ baseUrl, user, album }) {
	...
	return user ? (
		<>
			<Head>
				<title>
					{username} | Artwork in {album.name}{' '}
				</title>
			</Head>
			...
			<Space h='sm' />
			<Container size='xl'>
				<Title id='album-listing-title'>
					{album.name ? (
						<>
							Artwork in <em>{album.name}</em>
						</>
					) : (
						<>Artworks</>
					)}
